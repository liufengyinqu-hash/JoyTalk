...
impl MCURequestCRC {
    ...
    /// CRC for NFC `GetNFCData` requests (request id 2, padding 0xff).
    pub fn compute_nfc_crc8(&mut self) {
        self.crc = compute_crc8(0, &self.bytes);
        self._padding_0xff = 0xff;
    }

    /// CRC for `GetMCUStatus` / other empty MCU requests (padding 0xff).
    pub fn compute_mcu_request_crc8(&mut self) {
        self.crc = compute_crc8(0, &self.bytes);
        self._padding_0xff = 0xff;
    }
}
...
impl MCURequest {
    /// Build a `GetNFCData` MCU request with payload + CRC.
    pub fn nfc_data(payload: &[u8]) -> Self {
        let mut req: MCURequest = MCURequestEnum::GetNFCData(()).into();
        let crc = req.crc_mut();
        let copy_len = payload.len().min(36);
        crc.bytes[..copy_len].copy_from_slice(&payload[..copy_len]);
        crc.compute_nfc_crc8();
        req
    }

    /// Build a `GetMCUStatus` MCU request with CRC (required for MCU to respond).
    pub fn mcu_status_request() -> Self {
        let mut req: MCURequest = MCURequestEnum::GetMCUStatus(()).into();
        req.crc_mut().compute_mcu_request_crc8();
        req
    }

    /// Enable MCU payload in `0x31` input reports (output `0x11`, request id 3).
    /// `mode`: `0x00` camera, `0x01` config, `0x02` NFC/IR data, `0x03` IR-only.
    pub fn poll_stream(mode: u8) -> Self {
        let mut req: MCURequest = MCURequestEnum::GetIRData(IRRequest::from(IRRequestEnum::GetState(()))).into();
        req.crc_mut().bytes[0] = mode;
        req.crc_mut().compute_crc8(IRRequestId::GetState);
        req
    }
}

impl MCUReport {
    /// Raw MCU report bytes (312 bytes including report id).
    pub fn as_bytes(&self) -> &[u8; 312] {
        unsafe { &*(self as *const MCUReport as *const [u8; 312]) }
    }
}
