...
impl JoyConDevice {
    ...
    /// Open the HID handle only — no SPI calibration reads.
    /// Calibration is skipped intentionally: JoyTalk does not use stick/IMU
    /// factory calibration, and reading SPI right after a Bluetooth reconnect
    /// can take hundreds of milliseconds (or fail entirely) while the 60 Hz
    /// input-report stream is already active.
    pub fn open_hid(device_info: &DeviceInfo, hidapi: &HidApi) -> JoyConResult<HidDevice> {
        Self::check_type_of_device(device_info)?;
        let serial = device_info.serial_number().unwrap_or("");
        Ok(hidapi.open_serial(
    ...
    pub fn new(device_info: &DeviceInfo, hidapi: &HidApi) -> JoyConResult<Self> {
        ...
        let serial = device_info.serial_number().unwrap_or("");
        let hid_device = Self::open_hid(device_info, hidapi)?;

        Ok(JoyConDevice {
            hid_device: Some(hid_device),
            serial_number: serial.to_string(),
            device_type,
            stick_parameters: calibration::stick::StickParameters::from([0u8; 18]),
            stick_factory_calibration: calibration::stick::JoyConSticksCalibration::from(
                [0xFFu8; 18],
            ),
            stick_user_calibration: calibration::stick::JoyConSticksCalibration::from(
                [0xFFu8; 18],
            ),
            imu_offsets: calibration::imu::IMUOffsets::from([0u8; 6]),
            imu_factory_calibration: calibration::imu::IMUCalibration::from([0xFFu8; 24]),
            imu_user_calibration: calibration::imu::IMUCalibration::from([0xFFu8; 24]),
            color: color::Color::from([0u8; 12]),
        })
    }
