...
/// Portable mode support for JoyTalk.
///
...
pub fn init() {
    PORTABLE_DATA_DIR.get_or_init(|| {
        ...
        let is_portable = if is_valid_portable_marker(&marker_path) {
            ...
            let _ = std::fs::write(&marker_path, "JoyTalk Portable Mode");
            true
        } else {
            false
        };
...
fn is_valid_portable_marker(path: &std::path::Path) -> bool {
    std::fs::read_to_string(path)
        .map(|s| s.trim().starts_with("JoyTalk Portable Mode"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_valid_magic_string_enables_portable() {
        ...
        write!(f, "JoyTalk Portable Mode").unwrap();
